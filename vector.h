...
template <class T> void vector<T>::swap(int a, int b) {
    T aux = ptr[a];
    ptr[a] = ptr[b];
    ptr[b] = aux;
}
...
template <class T> void vector<T>::borrar_elemento(int i) {
    if (i < 0 || i > cant_elementos) {
        return;
    }
    else if (i == cant_elementos) {
        cant_elementos--;
        return;
    }
    else {
        for(int j = i; j < cant_elementos - 1; j++) {
            ptr[j] = ptr[j + 1];
        }
        cant_elementos--;
        return;
